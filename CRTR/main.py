...
from data.data_list_image import ImageList, ImageListIndex, rgb_loader,Normalize
...
def train(args, model):
    ...
    for global_step in range(1, t_total):
        ...
        if str(args.source_list) == 'data/SAR4class/r1m.txt':
            if accuracy >= 0.5:
                mask = torch.softmax(logits_t, dim=1).max(dim=1)[0] >= 0.4
                pseudo_labels = pseudo_labels[mask]
                feature_t = feature_t[mask]
